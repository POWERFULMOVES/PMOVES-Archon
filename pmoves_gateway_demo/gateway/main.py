          ...
          <style>
            ...
            *{ box-sizing:border-box }
            ...
            .orb{
              ...
              filter: blur(60px); opacity:.35; animation: float 16s ease-in-out infinite;
            }
            ...
            .card{
              position:relative; max-width:920px; width:92%; padding:32px; border-radius:16px;
              background: linear-gradient(180deg, #0b1222e6, #0b1222cc);
              border:1px solid #1f2a44; box-shadow: 0 10px 40px #0008, 0 0 60px var(--glow);
              backdrop-filter:saturate(140%) blur(10px);
            }
            .head{ display:flex; gap:16px; align-items:center; margin-bottom:10px }
            .logo{ width:48px; height:48px; display:grid; place-items:center; border-radius:12px;
                   background: conic-gradient(from 220deg, var(--acc), var(--acc2)); box-shadow: inset 0 0 20px #fff2; }
            ...
            .btn{
              text-decoration:none; color:var(--txt); border:1px solid #23304f; padding:16px 18px; border-radius:12px;
              background: linear-gradient(180deg, #0e1830, #0b1428);
              display:flex; align-items:center; gap:12px; transition: transform .15s ease, box-shadow .15s ease, border-color .15s;
            }
              ...
              <div class="head">
                <div class="logo" aria-hidden="true">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                    <path d="M12 3l3.5 6.1 6.5.9-4.7 4.6 1.1 6.4L12 18l-6.4 3.4 1.1-6.4L2 10l6.5-.9L12 3z"/>
                  </svg>
                </div>
                <div>
                  <h1>PMOVES.AI Gateway Demo</h1>
                  <div class="sub">FastAPI + WebRTC signaling endpoints with CHIT decoding utilities.</div>
                </div>
              </div>

              <section class="actions">
                <a class="btn" href="/demo/shapes-webrtc">
                  <svg class="icon" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M4 5h16a1 1 0 0 1 1 1v9.5a1 1 0 0 1-1 1H9l-4.5 3.5V6a1 1 0 0 1 1-1Z"/>
                  </svg>
                  <div>
                    <div>Shapes WebRTC Demo</div>
                    <div class="meta">Open this in two browser tabs</div>
                  </div>
                </a>

                <a class="btn" href="/web/client.html">
                  <svg class="icon" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M3 4h18v14H3zM7 20h10"/>
                  </svg>
                  <div>
                    <div>Client Test Page</div>
                    <div class="meta">Static client served from /web</div>
                  </div>
                </a>
              ...
              <section style="margin-top:24px">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px">
                  ...
                  <div style="border:1px solid #223457;border-radius:12px;padding:14px;background:#0b162b70">
                    ...
                    <details>
                      <summary style="cursor:pointer;color:var(--acc)">cURL equivalents</summary>
<pre style="margin-top:10px;white-space:pre-wrap"># Publish a CGP
curl -s http://localhost:8000/geometry/event \
  -H "content-type: application/json" \
  -d @tests/data/cgp_fixture.json

# Decode text
curl -s http://localhost:8000/geometry/decode/text \
  -H "content-type: application/json" \
  -d @tests/data/cgp_fixture.json

# Calibration report (writes artifacts/reconstruction_report.md on server)
curl -s http://localhost:8000/geometry/calibration/report \
  -H "content-type: application/json" \
  -d @tests/data/cgp_fixture.json
</pre>
                    </details>

...
export function validateCloneUrl(
  ...
  // `--upload-pack=<cmd>` -> arbitrary command execution). Ported from the
  // PMOVES fork guard; upstream validateCloneUrl does not cover dash-prefixed
  // sources because they fail the https regex and fall through unvalidated.
  // (CodeQL js/second-order-command-line-injection)
  if (cloneSource.startsWith('-')) {
    return { ok: false, error: 'Invalid repository URL: it must not start with a dash.' };
  }
